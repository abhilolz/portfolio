/*
Model: LEGO Space Dart I — Sketchfab CC-BY-NC-4.0
*/
...
export function Astronaut(props) {
  const group = useRef();
  const { nodes, materials } = useGLTF("/models/lego_space_dart_i.glb");

  useFrame((state) => {
    if (!group.current) return;
    const t = state.clock.getElapsedTime();

    // baseline position from Hero.jsx
    const baseY = props.position?.[1] || 0;

    // floating animation
    group.current.position.y = baseY + Math.sin(t * 1.2) * 0.06;

    // correct forward facing + gentle sway (NO flipping)
    group.current.rotation.set(
      0.15, // small nose-down tilt
      Math.PI + Math.sin(t * 0.4) * 0.08,
      0
    );
  });
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      {/* simplified transforms: stable + clean */}
      <group>
        ...
        />
        {/* ...keep ALL remaining mesh blocks exactly as before... */}
